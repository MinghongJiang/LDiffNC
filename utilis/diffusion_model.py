...
def diffusion_train(dataloader_train, dataloader_test, feature_dataloader_tr, dataset_info, dset_info, args):
    ...
    torch.manual_seed(123)
    # In utilis/diffusion_model.py, inside diffusion_train function

    # --- Add this section after loading/preparing feature_dataloader_tr ---
    print("Calculating class means for INC loss...")
    # Extract all features and labels from the TensorDataset
    # Assuming feature_dataloader_tr.dataset is a TensorDataset
    all_features = feature_dataloader_tr.dataset.tensors[0] # Shape: [num_samples, feature_dim]
    all_labels = feature_dataloader_tr.dataset.tensors[1]   # Shape: [num_samples]
    num_classes = dset_info["class_num"]
    feature_dim = all_features.shape[1]
    # Add channel dimension if needed by diffusion model (likely needed)
    # Match the unsqueeze(1) operation in the training loop
    # all_features = all_features.unsqueeze(1) # Shape: [num_samples, 1, feature_dim]
    # feature_dim_tuple = (1, feature_dim) # Store shape for mean calculation

    # Recalculate feature_dim if unsqueezed
    feature_dim_tuple = (1, feature_dim) # Diffusion model expects [batch, 1, seq_len]

    # Ensure features have the channel dimension for mean calculation
    if len(all_features.shape) == 2: # If shape is [N, D]
        all_features_unsqueezed = all_features.unsqueeze(1) # Shape: [N, 1, D]
    else: # Assuming shape is already [N, 1, D] or similar
        all_features_unsqueezed = all_features


    # Calculate Global Mean
    global_mean = torch.mean(all_features_unsqueezed.float(), dim=0, keepdim=True).to(device) # Shape: [1, 1, feature_dim]

    # Calculate Class Means
    class_means = torch.zeros(num_classes, *feature_dim_tuple, device=device) # Shape: [num_classes, 1, feature_dim]
    counts = torch.zeros(num_classes, device=device)

    for i in range(num_classes):
        class_mask = (all_labels == i)
        if class_mask.sum() > 0:
            class_features = all_features_unsqueezed[class_mask]
            class_means[i] = torch.mean(class_features.float(), dim=0)
            counts[i] = class_mask.sum()
        # Handle classes with 0 samples if necessary (e.g., leave as zeros or use global mean)

    print(f"Global mean shape: {global_mean.shape}")
    print(f"Class means shape: {class_means.shape}")
    # --- Calculation finished ---

    # --- Pass these means when initializing ConditionalDiffusion1D ---
    # (See modification in Step 3)
    ...
    if args.dataset == "CIFAR10" or args.dataset == "CIFAR100":
        ...
        diffusion_model = ConditionalDiffusion1D(
            ...
            objective='pred_x0',    
            # INC parameters 
            class_means=class_means, # means
            global_mean=global_mean, # global mean
            inc_intra_lambda=1e-4, # intra
            inc_inter_lambda=1e-4  # inter
    ...
    if args.is_diffusion_pretrained:
        ...
        for epoch in range(args.diffusion_epoch):
            ...
            for batch, data in enumerate(feature_dataloader_tr):
                ...
                diffusion_loss = diffusion_model(feature, 
                                                 feature_lable
    ...
    if args.dataset == "CIFAR10":
        ...
        for index, num in enumerate(dset_info['per_class_img_num']):
            if args.generation_mmf == "many":
                ...
                if index >= 7 and index < 10:
                    ...
                    class_num.append(0)
            elif args.generation_mmf == "new": #balance generate policy
                class_num.append(max(5000 - num, 5))
            else:
                class_num.append(max(round(num * args.feature_ratio), 5))
        ...
        for index, num in enumerate(dset_info['per_class_img_num']):
            if args.generation_mmf == "many":
                ...
                class_num.append(max(500 - num, 5))
            else:
                class_num.append(max(round(num * args.feature_ratio), 5))
    elif args.dataset == "ImageNet":
        class_num = []
